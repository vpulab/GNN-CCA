        ...
        if i >= 0:
            ...
            for g in range(len(len_graphs)):
                ...
                if flag_visualize:
                    ...
                    xws_1 = np.asarray([data_df[g]['xw'][data_df[g]['node'] == item].values for item in edge_ixs_g_np[0]])
                    yws_1 = np.asarray([data_df[g]['yw'][data_df[g]['node'] == item].values for item in edge_ixs_g_np[0]])
                    xws_2 = np.asarray([data_df[g]['xw'][data_df[g]['node'] == item].values for item in edge_ixs_g_np[1]])
                    yws_2 = np.asarray([data_df[g]['yw'][data_df[g]['node'] == item].values for item in edge_ixs_g_np[1]])
            ...
            if i >= 0:
                ...
                for g in range(len(len_graphs)):
                    ...
                    if flag_visualize:
                        ...
                        # coordinates of each pair of points
                        xws_1 = np.asarray(
                            [data_df[g]['xw'][data_df[g]['node'] == item].values for item in edge_ixs_g_np[0]])
                        yws_1 = np.asarray(
                            [data_df[g]['yw'][data_df[g]['node'] == item].values for item in edge_ixs_g_np[0]])
                        xws_2 = np.asarray(
                            [data_df[g]['xw'][data_df[g]['node'] == item].values for item in edge_ixs_g_np[1]])
                        yws_2 = np.asarray(
                            [data_df[g]['yw'][data_df[g]['node'] == item].values for item in edge_ixs_g_np[1]])
                        ...
                        spatial_dist_x = torch.abs(torch.from_numpy(xws_1 - xws_2)).cuda()
                        spatial_dist_y = torch.abs(torch.from_numpy(yws_1 - yws_2)).cuda()
                        edge_attr = torch.cat((spatial_dist_g.type(torch.float32), spatial_dist_x.type(torch.float32),
...
def validate_REID(val_loader, cnn_model):

    # RUN THIS TO VALIDATE THE WHOLE SEQUENCE AND OBTAIN THE OPTIMAL TH
    val_batch_time = utils.AverageMeter('batch_time', ':6.3f')
    ...
    reid_distances = np.concatenate(reid_distances)
    edge_label = np.concatenate(edge_label)
    return reid_distances, edge_label

def validate_REID_with_th(val_loader, cnn_model, th,max_v):
        ...
        for i, data in enumerate(val_loader):
            if i >= 0:
                ...
                for g in range(len(len_graphs)):
                    ...
                    # features reid distances between each pair of points
                    reid_distances = (F.pairwise_distance(reid_embeds[edge_ixs_g[0]], reid_embeds[edge_ixs_g[1]]).cpu().numpy())
...
def validate_GNN_cross_camera_association(CONFIG, val_loader, cnn_model, mpn_model):
            ...
            if i >= 0 :
                ...
                if CONFIG['ROUNDING']:
                   new_predictions = utils.compute_rounding(data_batch, predictions.view(-1), preds_prob)
                    ...
                    # projector = utils.GreedyProjector(subseq_graph)
    ...
    return P_list, R_list, F_list, TP_list, FP_list, FN_list, TN_list, rand_index, rand_index_rounding, P_r_list, R_r_list, F_r_list, TP_r_list, FP_r_list, FN_r_list, TN_r_list


def validate_TOPDB(val_loader, model,CONFIG):
    val_batch_time = utils.AverageMeter('batch_time', ':6.3f')
