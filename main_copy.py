...
from train import train, validate, validate_REID, compute_P_R_F
from train import validate_GNN_cross_camera_association, validate_TOPDB,validate_REID_with_th
...
if CONFIG['MODE'] == 'GNN':
    ...
    reid_dists, labels = validate_REID(validation_loader, cnn_model)
    precisions = []
    precisions_1 = []
    precisions_0 = []
    P_list = []
    ...
    P_list, R_list, F_list, TP_list, FP_list, FN_list, TN_list, rand_index, rand_index_rounding, P_r_list, R_r_list, F_r_list, TP_r_list, FP_r_list, FN_r_list, TN_r_list    = validate_GNN_cross_camera_association(CONFIG, validation_loader, cnn_model, mpn_model)
    ...
    if CONFIG['TOP_DB_WEIGHTS'] == 'market':
        model = bdnet(num_classes=751,  loss='softmax',  pretrained=True,  use_gpu= True, feature_extractor = False  )
        model.cuda()
        load_pretrained_weights(model, CONFIG['PRETRAINED_TOPDB_MODEL_MARKET'])

    elif CONFIG['TOP_DB_WEIGHTS'] == 'cuhk':
        model = top_bdnet_neck_botdropfeat_doubot(num_classes=767,  loss='softmax',  pretrained=True,  use_gpu= True, feature_extractor = False  )
        model.cuda()
        load_pretrained_weights(model, CONFIG['PRETRAINED_TOPDB_MODEL_CUHK'])


    P_list, R_list, F_list, TP_list, FP_list, FN_list, TN_list, r_index = validate_TOPDB(validation_loader, model, CONFIG)
    ...
    P_list, R_list, F_list, TP_list, FP_list, FN_list, TN_list, r_index = validate_REID_with_th(validation_loader, cnn_model, th_LAB, max_LAB)
    P = np.mean(np.asarray(P_list))
    R = np.mean(np.asarray(R_list))
    F = np.mean(np.asarray(F_list))
    TP = np.sum(np.asarray(TP_list))
