...
# import Arcface, Cosface, AMSoftmax, CircleLoss
...
class Baseline(nn.Module):
    ...
    def __init__(self, num_classes, last_stride, model_path, neck, neck_feat, model_name, pretrain_choice, cfg):
        ...
        if self.ID_LOSS_TYPE == 'arcface':
            ...
            self.classifier = metric_learning.Arcface(self.in_planes, self.num_classes,
                                      s=cfg.SOLVER.COSINE_SCALE, m=cfg.SOLVER.COSINE_MARGIN)
        elif self.ID_LOSS_TYPE == 'cosface':
            print('using {}'.format(self.ID_LOSS_TYPE))
            self.classifier = metric_learning.Cosface(self.in_planes, self.num_classes,
                                      s=cfg.SOLVER.COSINE_SCALE, m=cfg.SOLVER.COSINE_MARGIN)
        elif self.ID_LOSS_TYPE == 'amsoftmax':
            print('using {}'.format(self.ID_LOSS_TYPE))
            self.classifier = metric_learning.AMSoftmax(self.in_planes, self.num_classes,
                                        s=cfg.SOLVER.COSINE_SCALE, m=cfg.SOLVER.COSINE_MARGIN)
        elif self.ID_LOSS_TYPE == 'circle':
            print('using {}'.format(self.ID_LOSS_TYPE))
            self.classifier = metric_learning.CircleLoss(self.in_planes, self.num_classes,
                                        s=64, m=0.35)
        else:
    ...
    def load_pretrained_weights(self, trained_path):
        ...
        for (k, v), (k2,v2) in zip(param_dict.items(), model_dict.items()):
        ...
        if len(matched_layers) == 0:
            warnings.warn(
                ...
                '(** ignored and continue **)'.format(trained_path))
        else:
            print('Successfully loaded pretrained weights from "{}"'.format(trained_path))
