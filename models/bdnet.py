...
class TopBDNet(nn.Module):
    """
    """
    def __init__(self, num_classes=0, loss='softmax', neck = False, drop_height_ratio=0.33, drop_width_ratio=1.0, double_bottleneck=False, drop_bottleneck_features=False, feature_extractor=False,**kwargs):
    ...
    def forward(self, x, return_featuremaps = False, drop_top=False, visdrop=False):
        ...
        #global: just reduce dimensions from x: 2048 to x_x: 512
        ...
        #db
        drop_x = self.maxpool(drop_x).view(drop_x.size()[:2]) # 2048 vector
        t_drop_x = self.reduction_db(drop_x) # 1024 vector
